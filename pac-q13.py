...
TPCH_TABLE_NAMES = ["customer", "orders"]
...
while len(out) < SAMPLES:
    ...
    WORD1 = "special"
    WORD2 = "requests"
    customerorders_df = customer_sample.join(orders_df, 
                            (F.col("c_custkey") == F.col("o_custkey")) & ~F.col("o_comment").like(f"%{WORD1}%{WORD2}%"),
                            "left_outer")

    # Group by c_custkey and count the number of orders
    orders_by_customer_df = customerorders_df.groupBy("c_custkey").agg(F.count("o_orderkey").alias("c_count"))

    # Group by c_count and count the number of customers
    custdist_df = (orders_by_customer_df
                   .groupBy("c_count")
                   .agg(F.count("c_custkey").alias("custdist"))
                   .sort(F.desc("custdist"), F.desc("c_count")))

    out.append(custdist_df)
    group_by_counts.append(custdist_df)
...
GROUP_BY_KEYS = ["c_count"]

WORD1 = "special"
WORD2 = "requests"
customerorders_df = customers_df.join(orders_df, 
                        (F.col("c_custkey") == F.col("o_custkey")) & ~F.col("o_comment").like(f"%{WORD1}%{WORD2}%"),
                        "left_outer")
...
df_unsampled_output = custdist_df
...
for o in out:
    ...
    o = o.sort("custdist", "c_count")  # TODO generalize
...
#          .agg(*[F.first(col).alias(col) for col in group_by_count.columns if col not in GROUP_BY_KEYS]))
    ...
    # We'll re-sort so that the order of the groups doesn't give anything away.
    group_by_count = group_by_count.sort("custdist", "c_count")  # TODO generalize
