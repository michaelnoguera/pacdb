...
INPUT_FILE = 'tpch_q1.csv'  # has header row, ignore
presampled = open(INPUT_FILE, "r").readlines()[1:]
...
dfout = PACDataFrame.estimate_hybrid_noise_static(sample_once=hybrid_sampler.__next__, max_mi=1, anisotropic=False)

print(dfout)

exit()

HYBRID_PATH = "tpch_q1/hybrid.csv"
...
ANISOTROPIC_PATH = "tpch_q1/anisotropic.csv"
...
for c in categories:
    if os.path.exists('tpch_q1/anisotropic_{c}.pkl'):
        os.remove('tpch_q1/anisotropic_{c}.pkl')
    dfs_anisotropic[c].to_pickle(f'tpch_q1/anisotropic_{c}.pkl')
...
for c in categories:
    if os.path.exists('tpch_q1/hybrid_{c}.pkl'):
        os.remove('tpch_q1/hybrid_{c}.pkl')
    dfs_hybrid[c].to_pickle(f'tpch_q1/hybrid_{c}.pkl')