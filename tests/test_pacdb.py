...
def test_unwrap_df_scalar(spark_fixture):
    ...
    def query(df:DataFrame, sampling_rate = 1.) -> DataFrame:
...
def test_unwrap_df_vector(spark_fixture):
    ...
    assert np.array_equiv(PACDataFrame._unwrapDataFrame(query(df), "guardian"), np.array([90, 273, 32]))
...
def test_unwrap_df_matrix(spark_fixture):
    ...
    answer = PACDataFrame._unwrapDataFrame(query(df), "guardian")
